...
void Bishop::Move(std::vector<Model::Cell> &v, Model::Board* boardPtr, View::BoardView* boardViewPtr, int cellPositionY, int cellPositionX)
{
	...
	for (int i = 0; i < 4; i++)
	{
		*countPtr = 0;
		while (*isTruePtr)
		{
			if (isInsideBoard(cellPositionY + (*countPtr * stepCoiffLateralDirection[i][0]), cellPositionX + (*countPtr * stepCoiffLateralDirection[i][1])))
				*countPtr += 1;
			else
				break;
		}
	...
	for (int i = 0; i < 4; i++)
	{
		if (maxStepBishop[i] != 0)
			...
			{
				if (isFilledCell(cellPositionY + (j * stepCoiffLateralDirection[i][0]), cellPositionX + (j * stepCoiffLateralDirection[i][1]), boardPtr))
				{
